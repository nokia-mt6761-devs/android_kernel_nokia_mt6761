...
static struct imgsensor_info_struct imgsensor_info = { 
    ...
    .cap1 = {                            
	 ...
	 //define for dual camera by anxiang.xiao 20190102
	  .custom1 = {
		.pclk = 600000000,
		.linelength = 6004, 	
		.framelength = 3331, 
	...
	.ae_ispGain_delay_frame = 2,
#endif
	.frame_time_delay_frame = 1,//  The delay frame of setting frame length //define for dual camera by anxiang.xiao 20190102
	.ihdr_support = 0,      //1, support; 0,not support
	.ihdr_le_firstline = 0,  //1,le first ; 0, se first
	.sensor_mode_num = 7,      //support sensor mode num , //define for dual camera by anxiang.xiao 20190102
    ...
    .custom1_delay_frame = 2, //define for dual camera by anxiang.xiao 20190102
	...
	.i2c_addr_table = {0x40, 0xff}, //dyt
	//.i2c_addr_table = {0x46, 0xff}, //Q-tech
	.i2c_speed = 400,
};
...
//full size current fps : 24fps for PIP, 30fps for Normal or ZSD
	...
	.ihdr_en = 0,
	//.i2c_write_id = 0x46, //sunny
	.i2c_write_id = 0x40, //sunny
};
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{ 4224, 3136,   0,   6, 4224, 3124,	 4224, 3124,  8,  2, 4208, 3120, 0, 0, 4208, 3120},		// VIDEO (4208 x 3120)
 	{ 4224, 3136,   0, 482, 4224, 2172,  1408,  724,  64, 2, 1280,  720, 0, 0, 1280,  720},       // high video (1280 x 720)
 	{ 4224, 3136,   0,   4, 4224, 3128,  2112, 1564,  96, 242, 1920, 1080, 0, 0, 1920, 1080},       // slim video (1920 x 1080)
	{ 4224, 3136,   0,   6, 4224, 3124,	 4224, 3124,  8,  2, 4208, 3120, 0, 0, 4208, 3120},
};
...
static void set_shutter_frame_length(kal_uint32 shutter, kal_uint16 frame_length)
{
	...
	LOG_INF("Exit! shutter =%d, framelength =%d/%d, dummy_line=%d, auto_extend=%d\n", shutter,
		imgsensor.frame_length, frame_length, dummy_line, read_cmos_sensor(0x0340));
...
//0x020e, 0x067b, //frame length lines
0x020e, 0x0d03, //frame length lines  // 30fps
...
write_cmos_sensor(0x020a, 0x0bb6); //coarse integ time
write_cmos_sensor(0x021c, 0x0008); //coarse integ time short for iHDR
write_cmos_sensor(0x0206, 0x05dd); //line length pck
//write_cmos_sensor(0x020e, 0x067b); //frame length lines
write_cmos_sensor(0x020e, 0x0d03); //frame length lines
...
0x020e, 0x0d03, //frame length lines
...
write_cmos_sensor(0x021c, 0x0008); //coarse integ time short for iHDR
write_cmos_sensor(0x0206, 0x05dd); //line length pck
//write_cmos_sensor(0x020e, 0x0cf4); //frame length lines
write_cmos_sensor(0x020e, 0x0d03); //frame length lines
...
#endif
//define for dual camera by anxiang.xiao 20190102  ,need to check from vendor later 
...
//write_cmos_sensor(0x020e, 0x033d); //frame length lines
...
0x0730, 0x770f, //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1) 
0x0732, 0xe1b0, //pll_cfg_mipi_b b13-11:100_isp_div1(1/5) b10-8:001_mipi_div1(1/2) b1-0:00_mipi_div2(1/1) 
0x1118, 0x01aa, //LSC r_win_y B[11]: Bit8 of y offset in start block when cropping. B[10:8] y start index of block when cropping. B[7:0] y offset in start block when cropping.
0x1200, 0x011f, //PDPC BYPASS : Dyna-DPC ON, PDEN flag OFF, PD-DPC ON
0x1204, 0x1c01, //PDPC DC Counting OFF, PD Around BYPASS OFF
0x1240, 0x0100, //pdpc_pd_cnt_max_value
0x0b20, 0x8200, //HBIN mode
...
0x102c, 0x1400, //mipi_tpd_sync
0x1010, 0x07d0, //mipi_vblank_delay
0x1012, 0x00f4, //mipi_ch0_hblank_delay
0x1014, 0x0038, //mipi_hblank_short_delay1
0x1016, 0x0038, //mipi_hblank_short_delay2
0x101a, 0x0038, //mipi_pd_hblank_delay
0x1038, 0x0000, //mipi_virtual_channel_ctrl
0x1042, 0x0008, //mipi_pd_sep_ctrl_h, mipi_pd_sep_ctrl_l
0x1048, 0x0080, //mipi_pd_max_col_size
0x1044, 0x0100, //mipi_pd_col_size
...
0x0222, 0x0c80, //y addr start dummy
0x022c, 0x0c89, //y addr end dummy
0x0224, 0x020c, //y addr start active
0x022e, 0x0a81, //y addr end active
0x0f04, 0x0060, //fmt x cropping
0x0f06, 0x0000, //fmt y cropping
0x023a, 0x2222, //y dummy size
0x0234, 0x3311, //y even/odd inc tobp
0x0238, 0x3311, //y even/odd inc active
...
0x0206, 0x05dd, //line length pck
0x020e, 0x067b, //frame length lines
0x0b12, 0x0780, //x output size
0x0b14, 0x0438, //y output size
0x0204, 0x0200, //d2a_row_binning_en
0x041c, 0x0048, //pdaf patch start x-address 
0x041e, 0x1047, //pdaf patch end x-address 
0x0b04, 0x037e, //isp enable
...
static void slim_video_setting(void)
{
	...
	// frame rate = 60.24fps
// vblank length = 5793.86us
write_cmos_sensor(0x3250, 0xa470); //sreg8 - RAMP,B[15:10]:d2a_ramp_rng_ctrl
write_cmos_sensor(0x0730, 0x770f); //pll_cfg_mipi_a PLL_CLK=750mhz b7-6:00_isp_div2(1/1) 
write_cmos_sensor(0x0732, 0xe1b0); //pll_cfg_mipi_b b13-11:100_isp_div1(1/5) b10-8:001_mipi_div1(1/2) b1-0:00_mipi_div2(1/1) 
write_cmos_sensor(0x1118, 0x01aa); //LSC r_win_y B[11]: Bit8 of y offset in start block when cropping. B[10:8] y start index of block when cropping. B[7:0] y offset in start block when cropping.
write_cmos_sensor(0x1200, 0x011f); //PDPC BYPASS : Dyna-DPC ON, PDEN flag OFF, PD-DPC ON
write_cmos_sensor(0x1204, 0x1c01); //PDPC DC Counting OFF, PD Around BYPASS OFF
write_cmos_sensor(0x1240, 0x0100); //pdpc_pd_cnt_max_value
write_cmos_sensor(0x0b20, 0x8200); //HBIN mode
...
write_cmos_sensor(0x102c, 0x1400); //mipi_tpd_sync
write_cmos_sensor(0x1010, 0x07d0); //mipi_vblank_delay
write_cmos_sensor(0x1012, 0x00f4); //mipi_ch0_hblank_delay
write_cmos_sensor(0x1014, 0x0038); //mipi_hblank_short_delay1
write_cmos_sensor(0x1016, 0x0038); //mipi_hblank_short_delay2
write_cmos_sensor(0x101a, 0x0038); //mipi_pd_hblank_delay
write_cmos_sensor(0x1038, 0x0000); //mipi_virtual_channel_ctrl
write_cmos_sensor(0x1042, 0x0008); //mipi_pd_sep_ctrl_h, mipi_pd_sep_ctrl_l
write_cmos_sensor(0x1048, 0x0080); //mipi_pd_max_col_size
write_cmos_sensor(0x1044, 0x0100); //mipi_pd_col_size
...
write_cmos_sensor(0x0222, 0x0c80); //y addr start dummy
write_cmos_sensor(0x022c, 0x0c89); //y addr end dummy
write_cmos_sensor(0x0224, 0x020c); //y addr start active
write_cmos_sensor(0x022e, 0x0a81); //y addr end active
write_cmos_sensor(0x0f04, 0x0060); //fmt x cropping
write_cmos_sensor(0x0f06, 0x0000); //fmt y cropping
write_cmos_sensor(0x023a, 0x2222); //y dummy size
write_cmos_sensor(0x0234, 0x3311); //y even/odd inc tobp
write_cmos_sensor(0x0238, 0x3311); //y even/odd inc active
...
write_cmos_sensor(0x0206, 0x05dd); //line length pck
write_cmos_sensor(0x020e, 0x067b); //frame length lines
write_cmos_sensor(0x0b12, 0x0780); //x output size
write_cmos_sensor(0x0b14, 0x0438); //y output size
write_cmos_sensor(0x0204, 0x0200); //d2a_row_binning_en
write_cmos_sensor(0x041c, 0x0048); //pdaf patch start x-address 
write_cmos_sensor(0x041e, 0x1047); //pdaf patch end x-address 
write_cmos_sensor(0x0b04, 0x037e); //isp enable
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		LOG_INF("[Hi1336]preview\n");
		preview(image_window, sensor_config_data);
		break;
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		LOG_INF("[Hi1336]capture\n");
	//case MSDK_SCENARIO_ID_CAMERA_ZSD:
		capture(image_window, sensor_config_data);
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		LOG_INF("[Hi1336]video preview\n");
		normal_video(image_window, sensor_config_data);
		break;
	case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
		LOG_INF("[Hi1336]High speed vide ");
		hs_video(image_window, sensor_config_data);
		break;
	case MSDK_SCENARIO_ID_SLIM_VIDEO:
		LOG_INF("[Hi1336]SLIM speed vide ");
			...
			MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_SLIM_VIDEO:
		...
		if (imgsensor.frame_length > imgsensor.shutter)
			set_dummy();
	break;
	//add/define for dual camera by anxiang.xiao 20190102
		 case MSDK_SCENARIO_ID_CUSTOM1:
            ...
            imgsensor.min_frame_length = imgsensor.frame_length;
            spin_unlock(&imgsensor_drv_lock);
			if (imgsensor.frame_length > imgsensor.shutter)  //add/define for dual camera by anxiang.xiao 20190102
            set_dummy();
        	break;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		...
		LOG_INF("Hi1336 GET_PDAF_DATA EEPROM\n");
		// read from e2prom
#if e2prom
